	...
	tinymce.create('tinymce.plugins.VlsGfButtons', {
		...
		showAlbumSelectionDialog: function() {
			...
			editor.windowManager.open({
				...
				title: vlsGfTinymceL10n.strDialogTitle,
				buttons: [
					{
		...
		initModalContent: function(data, editor) {
			...
			body.find('li.vls-gf-folder>i').on('click', function() {
				//toggle folders
			...
			// allow viewer insertion only for "page" type
			if (vlsGfTinymceL10n.postType === 'page') {
				body.find('li.vls-gf-folder').on('click', function() {
					//select the album and insert its shortcode in the editor
					var id = $(this).data('id');
					editor.insertContent('[vls_gf_viewer id="' + id + '" header_tag="h2" header_class=""]');
